...
function iamportMethod (spec) {
  ...
  return function(param) {
    ...
    if(spec.urlParam2) {
      if( hasOwn.call(param, spec.urlParam2) ) {
        apiParams.push( param[spec.urlParam2] );
        param = _.omit(param, spec.urlParam2);
      }
      // urlParam2 is optional - no error if missing
    }
    ...
    if( requestMethod === 'POST' || requestMethod === 'PUT' ) formData = param;
