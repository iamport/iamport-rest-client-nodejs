...
module.exports = resource.extend({
  ...
  /** method 생성
   * OTP 본인인증을 요청합니다.
   * @see {@link https://api.iamport.kr/#!/certifications/requestOtp}
   *
   * @returns {promise} json 인증 정보
   * @public
   */
  otpRequest: iamportMethod({
    method: 'POST',
    command: 'otp/request',
    urlParam: null
  }),
  ...
  /** method 생성
   * OTP 본인인증 결과를 확인합니다.
   * @see {@link https://api.iamport.kr/#!/certifications/confirmOtp}
   *
   * @returns {promise} json 인증 정보
   * @public
   */
  otpConfirm: iamportMethod({
    method: 'POST',
    command: 'otp/confirm',
    urlParam: 'imp_uid'
  })
